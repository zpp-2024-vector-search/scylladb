...
# Some of the lines in these tests are commented out because Scylla doesnt support arithmetic operations in literals.
...
def test_invalidNumberOfDimensionsFixedWidth(cql, test_keyspace):
    with create_table(cql, test_keyspace,"(pk int primary key, value vector<int, 2>)") as table:
        ...
        assertInvalidThrowMessage(cql, table,  "Not enough bytes to read a vector", InvalidRequest,
                                    "INSERT INTO %s (pk, value) VALUES (0, ?)", [1])
...
@pytest.mark.skip(reason="The Python driver doesnt support variable width types in vectors yet")
def test_invalidNumberOfDimensionsVariableWidth(cql, test_keyspace):
...
@pytest.mark.skip(reason="The Python driver doesnt support variable width types in vectors yet")
def test_sandwichBetweenUDTs(cql, test_keyspace):
...
@pytest.mark.skip(reason="The Python driver doesnt support variable width types in vectors yet")
def test_invalidElementTypeVariableWidth(cql, test_keyspace):
    with create_table(cql, test_keyspace, "(pk int primary key, value vector<text, 2>)") as table:
        ...
        assertInvalidThrowMessage(cql, table, "bytes",
                                  InvalidRequest,
                                  "INSERT INTO %s (pk, value) VALUES (0, ?)", [1, 2])
        ...
        # assertInvalidThrowMessage(cql, table, "String didn't validate.",
        #                           InvalidRequest,
        #                           "INSERT INTO %s (pk, value) VALUES (0, ?)",
...
def test_emptyValues(cql, test_keyspace):
    def assertRejectsEmptyValues(t):
        with create_table(cql, test_keyspace, f"(k int primary key, v vector<{t}, 2>)") as table:
            assertInvalidThrowMessage(cql, table, "Invalid",
                                      InvalidRequest,
...
# Test commented out because Scylla doesnt support token(*literal*) syntax.
...
# FIXME Scylla doesnt support java as a language for UDFs, so this test is commented out.
# @Test
