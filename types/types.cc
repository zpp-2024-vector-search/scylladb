...
// Split on delimiter, unless it's preceded by '\'.
static std::vector<std::string_view> split_field_strings(std::string_view v, char delimiter) {
    if (v.empty()) {
...
// Concat list of bytes into a single bytes without length prefix.
static bytes concat_fields(const std::vector<bytes>& fields) {
    std::size_t result_size = 4 * fields.size();
    bytes result{bytes::initialized_later(), result_size};
    bytes::iterator it = result.begin();
    for (const bytes& field : fields) {
        it = std::copy(std::begin(field), std::end(field), it);
    }
    return result;
}
...
struct from_string_visitor {
    ...
    bytes operator()(const vector_type_impl& t) {
        std::vector<std::string_view> field_strings = split_field_strings(s, ',');
        if (field_strings.size() > t.get_dimension()) {
            throw marshal_exception(
                    format("Invalid vector literal: too many elements. Type {} expects {:d} but got {:d}",
                            t.as_cql3_type(), t.get_dimension(), field_strings.size()));
        }
        std::vector<bytes> fields(field_strings.size());
        std::vector<int32_t> field_len(field_strings.size());
        for (std::size_t i = 0; i < field_strings.size(); ++i) {
            field_strings[i] = trim(field_strings[i]);
            fields[i] = t.get_elements_type()->from_string(field_strings[i]);
            field_len[i] = fields[i].size();
        }
        if (t.get_elements_type()->is_native()) {
