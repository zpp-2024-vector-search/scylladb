                                         ...
                                         std::unique_ptr<attributes> attrs)
{
    ...
    if (prepared_ann_ordering.has_value()) {
        ...
        auto it = std::find_if(indexes.begin(), indexes.end(), [&prepared_ann_ordering](const auto& ind) {
            return (ind.metadata().options().at(cql3::statements::index_target::custom_index_option_name) == "dummy-vector-backend")
        ...
        if (it != index_opt->metadata().options().end() && it->second == "dummy-vector-backend") {
                             ...
                             const query_options& options) const
{
    ...
    if (_prepared_ann_ordering.has_value()) {
        ...
        if (limit.has_value()) {
            if (limit.assume_value() > max_ann_query_limit) {
                co_await coroutine::return_exception(exceptions::invalid_request_exception(fmt::format("Use of ANN OF in an ORDER BY clause requires a LIMIT that is not greater than {}. LIMIT was {}", max_ann_query_limit, limit.value())));
            }
        } else {
            co_await coroutine::return_exception(std::move(limit).as_failure().error());
        }
